...
def get_model(model_provider_func):
    ...
    if args.deepspeed:
        #fitting models that occupy more than half of GPU memory
        #must be converted to half before moving it to the GPU
        model.half()
...
def backward_step(optimizer, model, loss):
    ...
    if args.deepspeed:
        ...
        #deepspeed also handles gradient clipping no need to do it here
        return
...
def evaluate(forward_step_func, data_iterator, model, verbose=False):
    ...
    with torch.no_grad():
        ...
        while iteration < args.eval_iters:
            ...
            if args.deepspeed:
                #the buffers that are reset by backward in normal training
                #must be reset manually when using deepspeed checkpointing
